...
export interface HCS10InitializationOptions {
    ...
    registryUrl?: string;
    demoState?: DemoState; // Make demoState optional for broader use
}
 ...
 */
export async function initializeHCS10Client(options?: HCS10InitializationOptions) {
    ...
    if (networkEnv === 'mainnet') {
        ...
        console.warn(`Unsupported network specified in HEDERA_NETWORK: '${networkEnv}'. Defaulting to 'testnet'.`);
    ...
    const connectionTool = new ConnectionTool({
        client: hcs10Client,
        demoState: options?.demoState as DemoState
    });
