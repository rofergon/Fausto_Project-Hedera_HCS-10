...
export class CheckMessagesTool extends StructuredTool {
    ...
    protected async _call({ targetIdentifier }: z.infer<this["schema"]>): Promise<string> {
        ...
        const lastProcessedTimestamp = this.demoState.getLastTimestamp(connectionTopicId);

        this.logger.info(`Checking messages for connection with ${targetAgentName} (${connection.targetAccountId}) on topic ${connectionTopicId} since timestamp ${lastProcessedTimestamp}`);
...
`;
            ...
            for (const msg of newMessages) {
                const msgTimestampNanos = msg.timestamp * 1_000_000;
                latestTimestampNanos = Math.max(latestTimestampNanos, msgTimestampNanos);
...
`;
            ...
            if (latestTimestampNanos > lastProcessedTimestamp) {
                this.logger.debug(`Updating timestamp for topic ${connectionTopicId} to ${latestTimestampNanos}`);
                this.demoState.updateTimestamp(connectionTopicId, latestTimestampNanos);
            }

            return outputString.trim();

        } catch (error) {
            this.logger.error(`Failed to check messages for topic ${connectionTopicId}: ${error}`);
