...
export class SendMessageToConnectionTool extends StructuredTool {
    ...
    description = "Sends a text message to another agent using an existing active connection. Identify the target agent using their account ID (e.g., 0.0.12345) or the connection number shown in 'list_connections'.";
    schema = z.object({
        targetIdentifier: z.string().describe("The account ID (e.g., 0.0.12345) of the target agent OR the connection number (e.g., '1', '2') from the 'list_connections' tool."),
    ...
    protected async _call({ targetIdentifier, message }: z.infer<this["schema"]>): Promise<string> {
        ...
        this.logger.info(`Sending message from ${operatorId} to ${targetAgentName} (${connection.targetAccountId}) via connection topic ${connectionTopicId}`);

        try {
            // Call sendMessage with correct arguments
            const statusString = await this.hcsClient.sendMessage(
                connectionTopicId,
                operatorId,         // Sender's identifier
            ...
            this.logger.error(`Failed to send message via connection ${connectionTopicId}: ${error}`);
