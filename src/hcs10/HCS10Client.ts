...
export class HCS10Client {
    ...
    public async getInboundTopicId(): Promise<string> {
        try {
            ...
            console.log(`[HCS10Client] Retrieving profile for operator ${operatorId} to find inbound topic...`);
            ...
            if (profileResponse.success && profileResponse.topicInfo?.inboundTopic) {
                console.log(`[HCS10Client] Found inbound topic for operator ${operatorId}: ${profileResponse.topicInfo.inboundTopic}`);
                return profileResponse.topicInfo.inboundTopic;
            } else {
                throw new Error(`Could not retrieve inbound topic from profile for ${operatorId}. Profile success: ${profileResponse.success}, Error: ${profileResponse.error}`);
            }
        } catch (error) {
            console.error(`[HCS10Client] Error fetching operator's inbound topic ID (${this.getOperatorId()}):`, error);
